(async function() {
    ...
    async function initTracking() {
        ...
        try {
            ...
            if (result.success && Array.isArray(result.affiliate_url)) {
                // Iterate over the array of affiliate URLs and create tracking pixels for each
                result.affiliate_url.forEach((url) => {
                    createTrackingPixel(url);
                });
                sessionStorage.setItem('iframe_triggered', 'true'); 
            } else if (result.success && typeof result.affiliate_url === 'string') {
                // If it's a single URL (string), handle it as before
